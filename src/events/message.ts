...
export default messageEvent(Events.MessageCreate, ({ log }, msg) => {
  ...
  const pokemonNameArg: string = args[1];

  //call api to retrieve data
  pokeAPI.getPokemon(pokemonNameArg).then((response) => {
    const embed: EmbedBuilder = new EmbedBuilder()
    .setTitle(response.name)
    .setThumbnail(response.sprite)
    .addFields(
      {name: "Abilities", value: response.abilities.join(", ")},
      {name: "Stats", value:
      `**HP** ${response.stats.find(stat => stat.name === 'hp')?.value}\n` +
      `**Atk** ${response.stats.find(stat => stat.name === 'attack')?.value}\n` +
      `**Def** ${response.stats.find(stat => stat.name === 'defense')?.value}\n` +
      `**SpA** ${response.stats.find(stat => stat.name === 'special-attack')?.value}\n` +
      `**SpD** ${response.stats.find(stat => stat.name === 'special-defense')?.value}\n` +
      `**Spe** ${response.stats.find(stat => stat.name === 'speed')?.value}`}
    )
